...
int main(int argc, char **argv)
{
	...
	for (int i = 0; i < ite_count; i++)
	{
		...
		if (rank == size - 1)
		{
			MPICHECK(MPI_Recv(&receve_data, 4, MPI_INT, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE));
		}
	...
	for (int count = 4; count <= 2048; count *= 2)
	{
		...
		for (int i = 0; i < ite_count; i++)
		{
			...
			if (rank == size - 1)
			{
				MPICHECK(MPI_Recv(&receve_data, count, MPI_INT, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE));
			}
