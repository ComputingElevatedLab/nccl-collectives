...
int main(int argc, char* argv[])
{
    ...
    NCCL_CALL(ncclGroupStart());
    for (int i = 0; i < num_devices; i++) {
        // (int r, char* sendbuf, int sendcount, ncclDataType_t sendtype, char* recvbuf, int recvcount, ncclDataType_t recvtype,  ncclComm_t comm)
        ncclBruck(1, (char*) d_send_data[i], world_size, ncclFloat, (char*) d_recv_data[i], world_size, ncclFloat, comms[i]);
    }
    NCCL_CALL(ncclGroupEnd());
