...
int main(int argc, char *argv[])
{
  ...
  std::cout << __LINE__ << " rank " << mpi_rank << " send data " << h_send_data[0] << " " << h_send_data[1] << " " << h_send_data[2] << " " << h_send_data[3] << " " << h_send_data[4] << " " << h_send_data[5] << " " << h_send_data[6] << " " << h_send_data[7] << std::endl;
  ...
  CUDACHECK(cudaMemcpy(d_builtin_send_data, h_send_data, buffer_bytes, cudaMemcpyHostToDevice));

  // NCCL: Builtin
  int nRanks;
  NCCLCHECK(ncclCommCount(comm, &nRanks));
  std::cout << "rank " << mpi_rank << " commcount " << nRanks << std::endl;
  const size_t rankOffset = send_count * ncclTypeSize(ncclInt);
  NCCLCHECK(ncclGroupStart());
  for (int r = 0; r < nRanks; r++)
  {

    int src = (mpi_rank + r) % world_size;
    int dst = (mpi_rank - r + world_size) % world_size;
    NCCLCHECK(ncclSend(((char *)d_builtin_send_data) + dst * rankOffset, rankOffset, ncclInt, dst, comm, stream));
    NCCLCHECK(ncclRecv(((char *)d_builtin_recv_data) + src * rankOffset, rankOffset, ncclInt, src, comm, stream));
    // NCCLCHECK(ncclSend(((char *)d_builtin_send_data) + r * rankOffset, send_count, ncclInt, r, comm, stream));
    // NCCLCHECK(ncclRecv(((char *)d_builtin_recv_data) + r * rankOffset, send_count, ncclInt, r, comm, stream));
  }
  ...
  CUDACHECK(cudaMemcpy(h_nccl_builtin_recv_data, d_builtin_recv_data, buffer_bytes, cudaMemcpyDeviceToHost));
  CUDACHECK(cudaDeviceSynchronize());
  ...
  if (any_failed)
  {
    ...
    std::cout << "Iteration " << send_count << " - nccl-builtin - rank " << mpi_rank << ": " << result[passed[1]] << std::endl;
  }
  else
  {
